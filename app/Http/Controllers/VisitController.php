...
class VisitController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if(Cache::has($cache)){
            ...
            $query = Visit::with('user')
                ...
                ->pending(false) // Filtra por el estado de 'pending_visit'
                ->visited(false) // Filtra por el estado de 'visited'
                ->paginate(10); 
    ...
    public function pending(Request $request)
    {
        ...
        if(Cache::has($cache)){
            ...
            $query = Visit::with('user')
                ...
                ->orderBy('date_visit', 'ASC') // Ordena por la fecha de visita de forma ascendente
                ->where('date_visit', '>=', Carbon::today()) // Filtra por fechas de visita que sean hoy o futuras
                ->pending(true) // Filtra por el estado de 'pending_visit'
                ->paginate(10); 
    ...
    public function visited(Request $request)
    {
        ...
        if(Cache::has($cache)){
            ...
            $query = Visit::with('user')
                ...
                ->orderBy('date_visit', 'ASC') // Ordena por la fecha de visita de forma ascendente
                ->where('date_visit', '>=', Carbon::today()) // Filtra por fechas de visita que sean hoy o futuras
                ->visited(true) // Filtra por el estado de 'visitado'
                ->paginate(10); 
        ...
        // Renderiza inertia
        return Inertia::render('Auth/VisitVisited' ,[
            'auth' => Auth::user()->load('roles.permissions'),
            'visit' => $query, // Datos de la consulta
            'users' => User::all()->only(['id', 'name']),
        ]);
    ...
    {   
        try {
            ...
            // Retorna a la pagina principal
            return redirect(route("visit.pending"));
        } catch (\Throwable $e) {
            // Retorna a la pagina principal
            return redirect(route("visit.pending"));
        }
